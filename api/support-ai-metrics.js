...
function buildAlerts(metrics, supportAiReady) {
    const alerts = [];
...
export default async function handler(req, res) {
    ...
    try {
        ...
        console.error('❌ [SUPPORT AI METRICS] Erreur serveur:', error);
        return res.status(500).json({
            error: 'Erreur interne',
            code: 'SUPPORT_AI_METRICS_SERVER_ERROR'
        });
    }
}
