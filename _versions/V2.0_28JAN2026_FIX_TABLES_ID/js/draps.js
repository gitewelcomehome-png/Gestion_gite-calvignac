...
function calculerAEmmener(resaParGite, infosCouverture) {
    ...
    for (let i = 0; i < gites.length; i++) {
        ...
        if (!infos || infos.nbReservationsCouvertes === 0 || infos.reservations.length === 0 || keys.length === 0) {
            html += `
                <div style="background: var(--card); border: 3px solid ${color}; border-radius: 12px; box-shadow: 4px 4px 0 #2D3436; padding: 15px; flex: 1; min-width: 180px;">
                    ...
                    <p style="color: var(--text-secondary); font-size: 0.85rem; text-align: center; padding: 20px 0;">
        ...
        html += `
            <div style="background: var(--card); border: 3px solid ${color}; border-radius: 12px; box-shadow: 4px 4px 0 #2D3436; padding: 15px; flex: 1; min-width: 200px;">
                <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px; background: ${color}; border-radius: 8px; margin-bottom: 12px;">
                ...
                <div style="background: #f8f9fa; border-radius: 8px; padding: 10px; margin-bottom: 10px; text-align: center;">
                    <div style="font-size: 1.2rem; font-weight: 700; color: var(--text);">${nbResa} réservation${nbResa > 1 ? 's' : ''}</div>
                    <div style="font-size: 0.75rem; color: var(--text-secondary); margin-top: 3px;">${dateInfo}</div>
                </div>
                <ul style="font-size: 0.8rem; line-height: 1.6; margin: 0; padding-left: 20px; color: var(--text);">
...
function afficherResultatsSimulation(resaParGite, dateLimit) {
    ...
    for (let i = 0; i < gites.length; i++) {
        ...
        html += `
            <div style="background: var(--card); border: 3px solid ${color}; border-radius: 12px; box-shadow: 4px 4px 0 #2D3436; padding: 15px; flex: 1; min-width: 220px;">
                <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px; background: ${color}; border-radius: 8px; margin-bottom: 12px;">
                    <h4 style="margin: 0; font-size: 0.95rem; color: white; font-weight: 700; text-transform: uppercase;">
                        🏠 ${gite.name}
                    </h4>
                ...
                <div style="text-align: center; margin-bottom: 12px;">
                    <div style="font-size: 1.8rem; font-weight: 700; color: var(--text);">${resas.length}</div>
                    <div style="font-size: 0.75rem; color: var(--text-secondary); margin-top: 3px;">réservation${resas.length > 1 ? 's' : ''}</div>
                </div>
                ...
                ${totalACommander > 0 ? `
                    <ul style="font-size: 0.75rem; line-height: 1.5; margin: 0; padding-left: 20px; color: var(--text);">
