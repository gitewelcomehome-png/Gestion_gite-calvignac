...
-- ================================================================
-- FONCTION ONBOARDING (insertion sans cache PostgREST)
-- ================================================================
...
BEGIN
    ...
    FOR v_gite IN SELECT * FROM jsonb_array_elements(p_gites) LOOP
        INSERT INTO gites (organization_id, name, slug, icon, color, capacity, address)
        VALUES (v_org_id, v_gite->>'name', v_gite->>'slug', v_gite->>'icon', 
                v_gite->>'color', (v_gite->>'capacity')::INT, v_gite->>'address');
    END LOOP;
...
GRANT EXECUTE ON FUNCTION insert_onboarding_data TO anon;
