...
-- Mise à jour des données existantes en fonction du champ 'site'
UPDATE reservations 
SET synced_from = 'Airbnb'
WHERE synced_from IS NULL 
  AND (site ILIKE '%airbnb%');

UPDATE reservations 
SET synced_from = 'Abritel'
WHERE synced_from IS NULL 
  AND (site ILIKE '%abritel%' OR site ILIKE '%homelidays%');

UPDATE reservations 
SET synced_from = 'Gîtes de France (centrale)'
WHERE synced_from IS NULL 
  AND site ILIKE '%gîtes de france%';
